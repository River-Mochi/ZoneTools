    ...
    public sealed class LocaleZH_HANT : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts)
        {
            Dictionary<string, string> d = new Dictionary<string, string>
            {
                ...
                // Groups
                { m_Setting.GetOptionGroupLocaleID(Setting.kActionsGrp),        "保護" },
                ...
                { m_Setting.GetOptionGroupLocaleID(Setting.kUsageGrp),          "使用說明" },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.ProtectOccupiedCells)), "● 保護已占用格子（有建築）" },
                {
                    ...
                    "**[ ✓ ] 已啟用**，Zone Tools 不會移除有建築道路一側的分區格子。\n" +
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.ProtectZonedCells)), "● 保護已塗刷的分區格子" },
                {
                    ...
                    "**[ ✓ ] 已啟用**，Zone Tools 不會修改已塗刷 RCIO 格子（空置或已占用）。\n" +
                    "**[   ] 已關閉**，已塗刷的分區塊可能會被移除。"
                },

                // Compatibility
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.ContourIconText)), "等高線" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.OpenLog)),
                    "如果 **ZoneTools.log** 存在，就開啟它。\n" +
                    "如果日誌檔案還不存在，就改為開啟 **Logs** 資料夾。"
                },
